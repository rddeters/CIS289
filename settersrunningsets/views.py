"""
Program: settersrunningsets\views.py
Author: River Deters
...
def create_plot():
    ...
    df = pd.read_sql_query("SELECT * FROM Setters", conn)

    # Close the connection
    conn.close()

    # Handle missing values by filling with zeros
    df["running_sets"].fillna("0", inplace=True)

    # Convert "spikes" column to integers
    df["running_sets"] = df["running_sets"].astype(int)

    # Group the data by team and sum up the 'spikes' for each team
    grouped = df.groupby('team')['running_sets'].sum()
    ...
    plt.title('Total Number of Running Sets Per Team')
...
def settersrunningsets_view(request):
    plot = create_plot()
    return render(request, 'settersrunningsets/index.html', {'plot': plot})
